...
Figure~\ref{figure:journalPlotGroups} shows the journals, grouped by a k-means algorithm, creating 8 groups. The bottom right shows the names of the journals closest to the center of the group. While most names state the topic of the research field, some do not. These are  \textit{SAGE Open}: social and behavioural sciences and humanities, \textit{RSC Advances}: chemical sciences, and \textit{Orvosi hetilap}: experimental and clinical medicine. The k-means algorithm ran on the 300-dimensional set, while the plot shows the 2-dimensional set. A digital version of this plot can be found at:~\url{https://goo.gl/tdHGnD}. The usage of this digital version is advised, since this interactive plot enables zooming and adjusting plot settings, which can give a better insight into the results than the presented figures.
\begin{landscape}
\begin{figure}
\begin{center}
